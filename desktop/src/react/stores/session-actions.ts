...
export async function createNewSession(): Promise<void> {
  ...
  try {
    ...
    const data = await res.json();
    const mode = data.defaultMode || data.mode || 'operate';
    window.dispatchEvent(new CustomEvent('hana-plan-mode', {
      detail: { enabled: mode === 'read_only', mode },
    }));
  } catch {
    window.dispatchEvent(new CustomEvent('hana-plan-mode', { detail: { enabled: false, mode: 'operate' } }));
  }
...
export async function cleanupArchivedSessions(maxAgeDays: number): Promise<{ deleted: number }> {
  try {
