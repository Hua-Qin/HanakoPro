...
import { createTerminalTools } from "../lib/tools/terminal-tools.js";
...
export class HanaEngine {
  ...
  async interruptSessionForPrompt(p) { return this._sessionCoord.interruptSessionForPrompt(p); }
  ...
  getInlineDiff() { return this._prefs.getInlineDiff(); }
  setInlineDiff(v) { this._prefs.setInlineDiff(v); }
  ...
  findCheckpointsBySessionSince(sessionPath, sinceTs) {
    return this._checkpointStore.findBySessionSince(sessionPath, sinceTs);
  }
  cleanupCheckpoints() {
    const cfg = this._prefs.getFileBackup();
  ...
  buildTools(cwd, customTools, opts = {}) {
    ...
    let result = createSandboxedTools(cwd, allTools, {
      ...
      getInlineDiffEnabled: () => this._readPreferences().inline_diff !== false,
    ...
    // 共享终端工具（与 desktop 终端窗口共用同一 PTY 池）
    // 走 wrapWithSessionPermission 后会按 session-permission-mode 分级：
    //   terminal_list / terminal_read 永远 allow
    //   terminal_create / terminal_write / terminal_interrupt / terminal_kill 在 ASK 模式弹许可
    result = {
      ...result,
      tools: [...result.tools, ...createTerminalTools({ defaultCwd: cwd })],
    };
