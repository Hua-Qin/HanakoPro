...
// ── 初始化日志服务（initDebugLog 为兼容层，实际指向 log-service） ──
...
app.route("/api", createLogsRoute());

// 终端 PTY（嵌入式终端窗口 + 未来 AI 工具共享）
const { restRoute: terminalRestRoute, wsRoute: terminalWsRoute } = createTerminalRoute({ upgradeWebSocket });
app.route("/api", terminalRestRoute);
app.route("/api", terminalWsRoute);
...
async function gracefulShutdown() {
  ...
  try {
    ...
    // 3.5 关闭所有 PTY 终端
    try { terminalManager.shutdown(); } catch {}
    dlog.log("server", "terminals closed");
  ...
  // 同步 flush 日志 buffer，确保关闭前的日志不丢失
  try { dlog.close(); } catch {}
  process.exit(0);
}
